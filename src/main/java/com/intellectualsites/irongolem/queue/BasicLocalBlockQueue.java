...
public abstract class BasicLocalBlockQueue extends LocalBlockQueue {
    ...
    @Override public abstract BlockData getBlock(int x, int y, int z);
    ...
    @Override public boolean setBlock(int x, int y, int z, BlockData data) {
        if ((y > 255) || (y < 0)) {
            return false;
        }
        int cx = x >> 4;
    ...
    public abstract class LocalChunk {
        ...
        public abstract void setBlock(final int x, final int y, final int z, final BlockData block);
    ...
    public class BasicLocalChunk extends LocalChunk {
        ...
        public BasicLocalChunk(BasicLocalBlockQueue parent, int x, int z) {
            super(parent, x, z);
            baseblocks = new BlockData[16][];
        }

        @Override public void setBlock(int x, int y, int z, BlockData block) {
            this.setInternal(x, y, z, block);
        }

        private void setInternal(final int x, final int y, final int z, final BlockData baseBlock) {
            final int i = QueueRestorationHandler.CACHE_I[y][x][z];
            final int j = QueueRestorationHandler.CACHE_J[y][x][z];
            BlockData[] array = baseblocks[i];
            if (array == null) {
                array = (baseblocks[i] = new BlockData[4096]);
            }
            array[j] = baseBlock;
        }
    }
