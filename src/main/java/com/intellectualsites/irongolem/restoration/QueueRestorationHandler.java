...
public class QueueRestorationHandler implements RestorationHandler {
    ...
    public QueueRestorationHandler(@NotNull final IronGolem plugin) {
        ...
        GlobalBlockQueue.IMP = new GlobalBlockQueue(
            QueueProvider.of(BukkitLocalQueue.class, BukkitLocalQueue.class),
            1, 65 /* TODO: Make configurable */);
        GlobalBlockQueue.IMP.runTask();
    }

    @Override public void restore(@NotNull final ChangeSource source, @NotNull final World world,
        @NotNull final Collection<Change> changes, @NotNull final Runnable completionTask) {
            ...
            @Override public void run(final List<Change> value) {
                for (final Change change : changes) {
                    ...
                    final Change newChange = new Change.ChangeBuilder()
                        ...
                        .withSubject(BlockSubject.of(change.getLocation().getBlock().getBlockData(), blockSubject.getFrom())).build();
                    value.add(newChange);
                }
            }
        };
